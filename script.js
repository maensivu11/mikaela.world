...
if(window.matchMedia('(hover:hover) and (pointer:fine)').matches){
  ...
  document.querySelectorAll('[data-reaction]').forEach(function(hit){var target=hit.dataset.reaction==='name'?document.querySelector('.name-gradient'):document.querySelector('.smiley-reaction');hit.addEventListener('pointerenter',function(){target.classList.add('is-active');});hit.addEventListener('pointerleave',function(){target.classList.remove('is-active');});});
}
